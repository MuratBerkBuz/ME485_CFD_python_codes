...
#-------------------------------------------------------------------------------------------------#
    def getFlux(self, normal, qcM, qcP):
      ...
      # elif self.flux == "HLLC" :
        ...
        # Compute HLLC flux
        sM = min(uM - np.sqrt(abs(self.gamma*pM/rM)), uP - np.sqrt(abs(self.gamma*pM/rM)))
        sP = max(uM + np.sqrt(abs(self.gamma*pM/rM)), uP + np.sqrt(abs(self.gamma*pM/rM)))

        # Compute HLLC flux terms
        fxM[0] = ruM; fxM[1] = ruM*uM + pM; fxM[2] = rvM*uM;      fxM[3] = uM*(reM + pM); 
        fyM[0] = rvM; fyM[1] = ruM*vM;      fyM[2] = rvM*vM + pM; fyM[3] = vM*(reM + pM); 
        ...
        # Compute HLLC flux
        flux = np.zeros(self.Nfields, float)
        if 0 <= sM:
            flux = fxM
        elif sM <= 0 <= sP:
            flux = (sP * fxM - sM * fxP + sM * sP * (qcP - qcM)) / (sP - sM +eps)
        elif 0 >= sP:
            flux = fxP
      # else:
...
#-------------------------------------------------------------------------------------------------#
    def minmaxVT(self, dqf, dqmin, dqmax):
        eps=1e-10
        lfunc = np.zeros(self.Nfields, float)
        for fld in range(self.Nfields):
    
            if dqf[fld] > 0:
                lfunc[fld] = (dqmax[fld]**2 + 2*dqmax[fld]*eps) / (dqmax[fld]**2 + dqf[fld]**2 + 2*dqmax[fld]*eps)
            elif dqf[fld] < 0:
                lfunc[fld] = (dqmin[fld]**2 + 2*dqmin[fld]*eps) / (dqmin[fld]**2 + dqf[fld]**2 + 2*dqmin[fld]*eps)
            else:
                lfunc[fld] = 1.0
            lfunc[fld] = min(lfunc[fld], 1)
...
#-------------------------------------------------------------------------------------------------#
    def reconstructFaceValues(self, qc, dqc, qb):
      ...
      elif(self.limiter=="VENKATAKRISHNAN"): ## Fill This Part
        # find the maximum and minumum of the stencils
        for elm, info in msh.Element.items():
          qemax = self.qc[elm,:] 
          qemin = self.qc[elm,:] 
          ...
          for face in range(nfaces):
            ...
            lfunc = np.minimum(lfunc, self.minmaxVT(dqf, dqmin, dqmax))
